...
const TemplateRoomBookOnhold = () => {
    const [template, setTemplate] = useState({})
    const [errors, setErrors] = useState({})
    const [errorMessage, setErrorMessage] = useState("")

    useEffect(() => {
        httpService.get(process.env.REACT_APP_rgsevaUrl + "/accom/getTemplateBookOnhold").then((response) => {
    ...
    const handleFormSubmit = event => {
        ...
        httpService.post(process.env.REACT_APP_rgsevaUrl + "/accom/saveTemplateBookOnhold", template).then((response) => {
            let data = response.data
            console.log(data)
    ...
    return (
        <div>
            <h5>Email template - room booking onhold</h5>
            <div>When the room booking is done successfully but the payment is not yet received i.e payment status "onhold" this email is send to the devotee.</div>
            <Form>
                <div className="mb-3">
...
export default TemplateRoomBookOnhold;